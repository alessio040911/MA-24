...
class LaunchWindow:
    def __init__(self, master):
        self.master = master
        self.master.title("Welcome to Casino Blackjack")
        self.master.geometry("1024x768")
        self.master.configure(bg="#003300")

        title = tk.Label(master, text="🎰 Welcome to Casino Blackjack 🎰",
                         font=("Playfair Display", 40, "bold"), fg="#FFD700", bg="#003300")
        title.pack(pady=80)

        subtitle = tk.Label(master, text="Click 'Launch' to start the game",
                            font=("Georgia", 24, "italic"), fg="#FFD700", bg="#003300")
        subtitle.pack(pady=10)

        launch_btn = tk.Button(master, text="Launch", font=("Georgia", 20, "bold"),
        ...
        quit_btn = tk.Button(master, text="Quit", font=("Georgia", 20, "bold"),
...
class BlackjackGame:
    def __init__(self, root):
        self.root = root
        root.title("Blackjack Casino Edition - Option 3 (Fixed)")
        ...
        # Variables
        self.player_score_var = tk.StringVar(value="Player Score: 0")
        self.dealer_score_var = tk.StringVar(value="Dealer Score: ?")
        self.result_var = tk.StringVar(value="Place your bet to start!")
        self.balance_var = tk.StringVar(value=f"Balance: ${self.balance}")
        self.bet_var = tk.StringVar(value=f"Current Bet: ${self.current_bet}")
    ...
    def load_cards(self):
        # Make ranks consistent strings so file names like "ace_spade.png" work.
    ...
    def setup_ui(self):
        ...
        # Dealer
        dealer_label = tk.Label(self.root, text="Dealer", font=self.score_font, fg="#ffe600", bg="#004225")
        dealer_label.pack()
        ...
        # Player
        player_label = tk.Label(self.root, text="Player", font=self.score_font, fg="#ffd700", bg="#004225")
        ...
        self.hit_button = tk.Button(button_frame, text="Hit", command=self.player_hit, **style_btn_opts)
        self.hit_button.grid(row=0, column=0, padx=12)
        self.stand_button = tk.Button(button_frame, text="Stand", command=self.player_stand, **style_btn_opts)
        self.stand_button.grid(row=0, column=1, padx=12)
        self.new_game_button = tk.Button(button_frame, text="New Game", command=self.new_game, **style_btn_opts)
        self.new_game_button.grid(row=0, column=2, padx=12)
        quit_btn = tk.Button(button_frame, text="Quit", command=self.root.destroy, **style_btn_opts)
    ...
    def change_bet(self, amount):
        ...
        if not self.game_over:
            self.result_var.set("Finish current round before changing bet.")
            return
        ...
        if amount > 0 and self.current_bet + amount > self.balance:
            self.result_var.set("Cannot bet more than balance!")
            return

        self.current_bet += amount
        self.bet_var.set(f"Current Bet: ${self.current_bet}")
        self.result_var.set("Bet updated. Click 'New Game' to deal.")
    ...
    def update_scores_display(self, hide_dealer_card=True):
        ...
        self.player_score_var.set(f"Player Score: {player_score}")
        if hide_dealer_card:
    ...
    def new_game(self):
        if self.current_bet == 0:
            self.result_var.set("Place a bet first!")
            return

        if not self.game_over:
            self.result_var.set("Round in progress! Finish it first.")
            return

        # Start a round
        self.game_over = False
        self.result_var.set("Dealing cards...")
    ...
    def resolve_game(self, player_score, dealer_score):
        ...
        self.game_over = True
        if player_score > 21:
            self.result_var.set("Bust! Dealer wins!")
            self.balance -= self.current_bet
        elif dealer_score > 21:
            self.result_var.set("Dealer busts! Player wins!")
            self.balance += self.current_bet
        elif dealer_score > player_score:
            self.result_var.set("Dealer wins!")
            self.balance -= self.current_bet
        elif dealer_score < player_score:
            self.result_var.set("Player wins!")
            self.balance += self.current_bet
        else:
            self.result_var.set("Push! It's a tie!")

        # Update UI state
        self.balance_var.set(f"Balance: ${self.balance}")
        self.bet_var.set(f"Current Bet: $0")
        self.current_bet = 0
